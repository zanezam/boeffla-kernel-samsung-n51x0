...
static struct mdnie_tuning_info *mdnie_request_table(struct mdnie_info *mdnie)
{
	...
	if (ACCESSIBILITY_IS_VALID(mdnie->accessibility)) {
		...
		table = &camera_table[mdnie->auto_brightness][mdnie->mode];
		goto exit;
...
static void update_color_position(struct mdnie_info *mdnie, u16 idx)
{
	...
	for (lux = 0; lux < LUX_MAX; lux++) {
		for (mode = 0; mode < MODE_MAX; mode++) {
			wbuf = camera_table[lux][mode].sequence;
			if (IS_ERR_OR_NULL(wbuf))
				continue;
			i = 0;
			while (wbuf[i] != END_SEQ) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	struct mdnie_tuning_info *table = NULL;
	const unsigned short *wbuf = NULL;
	char *lux[LUX_MAX] = {"0~9999", "10000~19999", "20000~39999", "40000~"};

	table = &camera_table[mdnie->auto_brightness][mdnie->mode];
	wbuf = table->sequence;

	pos += sprintf(pos, "auto_brightness: %d, scenario: %d, %s, lux is %s\n",
		mdnie->auto_brightness, mdnie->scenario, table->name, lux[mdnie->auto_brightness]);
	while (wbuf[i] != END_SEQ) {
		pos += sprintf(pos, "0x%04x, 0x%04x\n", wbuf[i], wbuf[i+1]);
		i += 2;
	}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (rc < 0)
		...
		if (mdnie->auto_brightness != value) {
			...
			value = (value >= LUX_MAX) ? LUX_10000 : value;

			mutex_lock(&mdnie->lock);
			mdnie->auto_brightness = value;
			mutex_unlock(&mdnie->lock);

			if (mdnie->enable && (mdnie->scenario == CAMERA_MODE))
				mdnie_update(mdnie);
		}
	}
	return size;
